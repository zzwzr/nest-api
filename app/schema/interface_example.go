...
func (InterfaceExample) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "interface_examples"},
	}
}

func (InterfaceExample) Mixin() []ent.Mixin {
	return []ent.Mixin{
		mixin.SoftDeleteMixin{},
	}
}

func (InterfaceExample) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("content_type").MaxLen(100).Default("application/json").Comment("Content-Type"),
		field.Text("raw").Default("").Comment("Raw 示例内容"),
...
func (InterfaceExample) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("interface", API.Type).Ref("response_examples").Unique().Required().Field("interface_id"),
	}
}
