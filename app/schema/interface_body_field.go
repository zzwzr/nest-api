...
type InterfaceBodyField struct {
	ent.Schema
}

func (InterfaceBodyField) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "interface_body_fields"},
	}
}

func (InterfaceBodyField) Mixin() []ent.Mixin {
	return []ent.Mixin{
		mixin.SoftDeleteMixin{},
	}
}

func (InterfaceBodyField) Fields() []ent.Field {
	return []ent.Field{
...
func (InterfaceBodyField) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("interface", API.Type).Ref("body_fields").Unique().Required().Field("interface_id"),
	}
}
